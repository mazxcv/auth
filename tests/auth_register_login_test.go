...
func TestRegisterLogin_HappyPath(t *testing.T) {
	...
	cl_app_id := int(claims["app_id"].(float64))
	assert.Equal(t, appID, cl_app_id, fmt.Sprintf("%d<>%d", appID, cl_app_id))

	const deltaSeconds = 1
	assert.InDelta(t, loginTime.Add(st.Cfg.TokenTTL).Unix(), claims["exp"].(float64), deltaSeconds)
}

func TestRegisterLogin_DuplicateRegistaration(t *testing.T) {
	ctx, st := suite.New(t)
	email := gofakeit.Email()
	pass := randomFakePass()
